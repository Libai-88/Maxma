...
class TokenBucketRegistry:
    ...
    def start_cleanup_task(self, interval: float = 300.0) -> None:
        """启动后台周期清理任务（每 interval 秒清理一次过期桶）。

        幂等：重复调用不会创建多个任务。需在 asyncio 事件循环内调用
        （通常由 server.py lifespan 启动）。
        """
        ...
        async def _run():
            while True:
                ...
                try:
                    ...
                    if removed:
                        logger.debug("[rate_limit] cleaned up %d idle bucket(s)", removed)
                except Exception as e:
                    logger.warning("[rate_limit] cleanup error: %s", e)
        self._cleanup_task = loop.create_task(_run())

    async def stop_cleanup_task(self) -> None:
        """停止后台清理任务（供 shutdown 调用）。"""
        if self._cleanup_task is not None and not self._cleanup_task.done():
...
class RateLimitMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # 首次调用时惰性启动 cleanup 任务（此时事件循环已就绪）
        if self._registry._cleanup_task is None:
            self._registry.start_cleanup_task()

...
Assert-Regex $server 'bun install --frozen-lockfile[^\r\n]*\|\| bun install[^\r\n]*\r?\n\s*if errorlevel 1\s*\(\s*echo \[ERROR\].*\s*exit /b 1\s*\)' "server build must stop when bun-sidecar preparation fails"

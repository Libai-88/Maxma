...
_BRAND_PROMPT = (
    ...
    "判断有分量但不喧哗（品牌取意「墨色」）。\n"
    # STICKER-PROMPT-001：此前"尽量带 1 处（纯技术/指令内容除外）"措辞过弱，
    # 技术问答场景被模型完全豁免，表情系统几乎不触发。改为明确的使用时机
    # （开场问候/情绪转折/收尾），只豁免纯代码/纯数据输出。
    "- 情感表达：在开场问候、情绪转折或收尾时，主动写入 1 处 [表情包:情绪]"
    "点缀（仅当整条回复为纯代码或纯数据输出时可省略），"

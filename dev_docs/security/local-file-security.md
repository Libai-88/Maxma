# MaxmaHere 本地文件读写安全机制

## 概述

MaxmaHere 采用**两层互补**的安全机制保护本地文件系统，防止 LLM 生成的代码或工具调用越权读写文件：

| 层级 | 机制 | 类型 | 粒度 | 定义方式 |
|------|------|------|------|----------|
| 第一层 | **MaxmaBlocker** | 黑名单式阻断 | 目录级 | 在目标目录放置标记文件 |
...
## 第一层：MaxmaBlocker（黑名单阻断）

### 原理

MaxmaBlocker 是一种基于**目录标记文件**的阻断机制。当某目录（或其任意级父目录）中存在文件名为 `MaxmaBlocker`（不区分大小写，不限制扩展名）的文件时，该目录及其所有子目录下的文件操作都会被拒绝。

这种方式允许用户在**不修改代码**的前提下，通过简单的"放置一个文件"来标记敏感目录。

### 实现

核心函数 `check_maxma_blocker()` 定义在 `tools/base.py:88`：
...
3. 每层目录中遍历所有条目，匹配忽略大小写和扩展名的 `MaxmaBlocker` 文件名
4. 一旦发现标记文件，**立即返回**该目录路径（阻断）
5. 全部检查通过 → 返回 `None`（允许）

> 为什么从根向下？因为用户可能在项目根目录放置 `MaxmaBlocker`，这应该拦截整个项目范围内的文件访问。

### 应用场景

- 用户在某些敏感目录（如包含密钥/配置的目录）中放置 `MaxmaBlocker` 文件，防止 LLM 意外读取
...
2. **先检 MaxmaBlocker** → 阻断则抛 `PermissionError`（Blocker 优先）
...
if blocked:
    raise PermissionError("🚫 安全阻断：操作已被 MaxmaBlocker 阻断。...")
...
```python
# 1. Blocker 优先
blocked = check_maxma_blocker(file_path)
if blocked:
    return format_error("🚫 安全阻断：操作已被 MaxmaBlocker 阻断。...")
...
这个优先级确保用户可以**一票否决**：即使路径在白名单内，只要放置了 `MaxmaBlocker` 文件就立即阻断。
...
if blocked:
    return format_error("🚫 安全阻断：操作已被 MaxmaBlocker 阻断。\n"
                        f"在目录 \"{blocked}\" 中发现了 MaxmaBlocker 文件。\n"
...
在要拦截的目录中创建任意文件，文件名忽略大小写为 `MaxmaBlocker`（可带任意扩展名）：

```bash
# 阻断整个文档目录
echo "DO NOT READ" > ~/Documents/MaxmaBlocker
# 也可带扩展名
echo "" > ./secrets/MaxmaBlocker.txt
```

### 移除阻断

删除对应目录下的 `MaxmaBlocker` 文件即可：

```bash
rm ~/Documents/MaxmaBlocker
```
